...
func (h *Handler) List(c echo.Context) error {
	...
	posts, total, err := h.svc.ListPublished(c.Request().Context(),
		c.QueryParam("section"), c.QueryParam("tag"), c.QueryParam("q"), p)
	if err != nil {
...
func (h *Handler) AdminList(c echo.Context) error {
	p := pagination.Parse(c)
	posts, total, err := h.svc.AdminList(c.Request().Context(),
		c.QueryParam("status"), c.QueryParam("section"), p)
	if err != nil {
		return response.InternalServerError(c, mw.GetRequestID(c))
	}
	return response.OKWithMeta(c, posts, pagination.NewMeta(p, total))
